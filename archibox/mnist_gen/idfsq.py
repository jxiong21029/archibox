...
class Config:
    ...
    savedir: str = str(Path(__file__).parent / "data/idfsq")
    ...
    muon_mu: float = 0.9
    muon_wd: float = 0.01
    scalar_lr: float = 0.0006
    embeds_lr: float = 0.0006
    output_lr: float = 0.0006
    adamw_mu1: float = 0.9
...
class VectorIDPrior(nn.Module):
    def __init__(
        ...
        self.pad_out_size = (((seq_len * vocab_size) + 127) // 128) * 128
        self.head = make_output_head(dim, self.pad_out_size)

    def loss(self, x_ND, token_ids_NL):
        for block in self.blocks:
            x_ND = x_ND + block(x_ND)
        logits = self.head(x_ND)
        logits = logits[..., : self.seq_len * self.vocab_size]
        logits = rearrange(
            logits, "... (L D) -> ... L D", L=self.seq_len, D=self.vocab_size
        )
        loss = F.cross_entropy(
            logits.flatten(0, -2), token_ids_NL.flatten(), reduction="none"
        )
        return loss
