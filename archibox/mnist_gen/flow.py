...
class MLPConfig(BaseModel):
    ...
    mlp_dim: int = 2048
    freq_dim: int = 256
    freq_range: tuple[float, float] = (0.1, 100.0)
    depth: int = 4
    use_adaln: bool = True


SAVEDIR = Path(__file__).parent / "data/flow"
...
class SimpleMLPNet(nn.Module):
    def __init__(self, cfg: MLPConfig):
        ...
        self.time_freqs = nn.Buffer(make_frequencies(cfg.freq_range, cfg.freq_dim))
        self.emb_proj = FusedLinear(cfg.freq_dim * 2, cfg.dim)
    ...
    def forward(self, input_ND, t_N1, cond_ND):
        x_ND = self.in_proj(input_ND)
        emb = fourier_embed(t_N1, self.time_freqs).type_as(x_ND)
...
class Flow(nn.Module):
    ...
    def loss(self, inputs_ND: Tensor, cond_ND: Tensor) -> Tensor:
        ...
        if self.cfg.schedule == "trig":
            ...
            xt = (1 - t) * x + t * e
            vt = -x + e
        u = self.net(xt.type(dtype), t, cond_ND)
        loss = (u - vt).pow(2)
    ...
    def sample(self, cond_input: Tensor) -> Tensor:
        ...
        for i in range(self.cfg.sampling_steps):
            ut = self.net(xt, timesteps[i], cond_input)
